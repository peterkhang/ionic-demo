...
public class DatePickerPlugin extends CordovaPlugin {
	...
	public boolean execute(final String action, final JSONArray data, final CallbackContext callbackContext) {
		d(pluginName, "DatePicker called with options: " + data);
		called = false;
		canceled = false;
	...
	private Runnable runnableDatePicker(
		...
		return new Runnable() {
			...
			public void run() {
				...
				final DatePickerDialog dateDialog = new ZoeDatePickerDialog(currentCtx, theme, dateSetListener, jsonDate.year,
		...
		if(startDate !=null || endDate != null) {
			pickerView.init(jsonDate.year, jsonDate.month, jsonDate.day, new OnDateChangedListener() {
				...
				public void onDateChanged(DatePicker view, int year, int month, int day) {
                	Log.d("onDateChanged", year + "/" + month + "/" + day);
                	if(jsonDate.maxDate > 0 && jsonDate.maxDate > jsonDate.minDate) {
	...
	private final class DateSetListener implements OnDateSetListener {
      ...
      this.theme = theme;
		...
		public void onDateSet(final DatePicker view, final int year, final int monthOfYear, final int dayOfMonth) {
			...
			d("onDateSet", "called: " + called);
			d("onDateSet", "canceled: " + canceled);
			d("onDateSet", "mode: " + jsonDate.action);
			
			if (ACTION_DATE.equalsIgnoreCase(jsonDate.action)) {
				String returnDate = year + "/" + (monthOfYear + 1) + "/" + dayOfMonth;
				d("onDateSet", "returnDate: " + returnDate);
